...
INSERT INTO `average_sales_optionalproduct_per_servicepackage` VALUES (13,0);
...
INSERT INTO `bestseller_optionalproduct` VALUES (1,NULL,NULL);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `add_failed_attempt` BEFORE UPDATE ON `orders` FOR EACH ROW begin
...
declare email2 varchar(64);
	...
	if new.orderState <=> "Rejected" and FailedAttempts <=> 2 then
	insert into suspended_orders(username,amount,lastRejectionDateTime,email) values (new.username,new.totalFee,current_timestamp(),email2);
    update users
    set isInsolvent = 1 where username = new.username;
    end if;
    if new.orderState <=>"Rejected" then
    update users
    set FailedAttempts = FailedAttempts + 1 where username = new.username;
    end if;
end */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `insert_sales_purchases_per_package` AFTER UPDATE ON `orders` FOR EACH ROW begin
declare packageId2 int;
declare monthlyFee2 float;
declare validityPeriod2 int;
		select
...
declare newProducts int;
    ...
    select count(*) into newProducts from order_optionalproduct where orderId = new.orderId;
	if not(old.orderState <=> new.orderstate) and new.orderstate <=> "Paid" then
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `insert_average_sales_optionalproduct_per_servicepackage` AFTER UPDATE ON `orders` FOR EACH ROW begin
declare packageId2 int;
declare totalPackage int;
declare newProducts int;
	select packageId into packageId2 from period where new.periodId = ID;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `insert_suspended_orders` AFTER UPDATE ON `orders` FOR EACH ROW begin
	if new.orderState <=> "Rejected" then
...
INSERT INTO `period` VALUES (1,10,10,13),(2,20,20,13);
...
INSERT INTO `servicepackage` VALUES (13,'ProvaPeriod');
...
INSERT INTO `servicepackage_optionalproduct` VALUES (13,1);
...
INSERT INTO `servicepackage_service` VALUES (13,1);
...
declare unpaid int;
    ...
    select count(*) into unpaid from orders where username = username2 and orderStatus = "Rejected";
	if unpaid <=> 0 then
    update users
    set isInsolvent = 1 , FailedAttempts = 0 where username = username2;
    end if;
end */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
INSERT INTO `total_purchases_per_package` VALUES (13,0);
...
INSERT INTO `total_purchases_per_package_validityperiod` VALUES (1,0),(2,0);
...
INSERT INTO `total_sales_per_package` VALUES (13,0,0);
...
INSERT INTO `users` VALUES ('andrea','1234','andrea@andrea.it',0,0),('gianluca','1234','gianluca@gianluca.it',0,0);
...
-- Dump completed on 2022-05-17 11:30:19
